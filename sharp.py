...
import string
import datetime
from config import 8091709404:AAH_TwGTOuNw6iyECX_QXqrhY1f4zu2WbfU, 7825026453, @ITS_TUFAN_01
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users or datetime.datetime.now() > datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S'):
        await update.message.reply_text("❌ Access expired or unauthorized. Please redeem a valid key. Buy key from @ITS_TUFAN_01\nCLICK IN /help")
        return
    ...
    await update.message.reply_text(f'Attack started:\nFlooding {target_ip}:{port} for {duration} seconds using sharp binary, UDP flood, and Super UDP. OWNER- @ITS_TUFAN_01')
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in user_processes or user_processes[user_id]["process"].poll() is not None:
        await update.message.reply_text('No flooding process is running.OWNER @ITS_TUFAN_01\nCLICK IN /help')
        return
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        ...
        response = "ONLY OWNER CAN USE💀OWNER @ITS_TUFAN_01"
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            response = "Invalid or expired key buy from @ITS_TUFAN_01."
    else:
...
async def owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📞 Contact the owner: @ITS_TUFAN_01")
